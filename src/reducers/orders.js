    ...
    case MOVE_ORDER_NEXT:
      return state.map(order => {
        if (order.id === action.payload) {
          const indexPositions = positions.findIndex(elem => elem === order.position)
          if (positions[indexPositions + 1] !== 'finish'){
            order.position = positions[indexPositions + 1]
          } else {
            const finishOrder = order.ingredients.filter(ing => order.recipe.includes(ing))
    ...
    case ADD_INGREDIENT:
      ...
      return state.map(order => {
        if (search && order.position === action.payload.from) {
          if (order.recipe.includes(action.payload.ingredient) 
